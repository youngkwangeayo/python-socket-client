...
class BaristaCall :
    ...
    def __init__(self, frId, url:str=None,signageUrl:str =None) -> None:
        self.socket = None
        self.frId = frId
        if url is None :
        ...
        if signageUrl is None :
            self.signageUrl = "https://dev-waiting.nextpay.co.kr/"
        else :
        ...
        while True:
            ...
            try:
                ...
                if "STATUS" in reqData : 
                    ...
                    # await self.httpRequestQueue.put(sendWating)
                    print("추가된거니?")
            except:
                print("입섹션")
    ...
    async def signageSendWatingInfo(self) -> None:
        while True:
            ...
            try :
                ...
                print("꺼내와 임마")
                url = self.signageUrl + "signageWatingInfo"
                url = "http://192.168.0.136:8112/testWathing"
                res = requests.post(url=url,headers={"Content-type": "application/json; charset=utf-8"},data=json.dumps(sendWating))
                print(f"http return : {res.status_code},  body : {res.json()}")
